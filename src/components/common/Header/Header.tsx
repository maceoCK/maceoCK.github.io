...
const Header: React.FC<HeaderProps> = ({ aboutRef, projectsRef, contactRef, topRef }) => {
    const scrollToRef = (ref: any) => {
        ref.current.scrollIntoView({ behavior: 'smooth' });
    };
    return (
        <AppBar position="static" style={{ position: 'fixed', top: 0, left: 0, right: 0, zIndex: 100, backgroundColor: '#ffe7c4', boxShadow: 'none' }}>
            <Toolbar>
                <div className='title' onClick={() => scrollToRef(topRef)} style={{ flexGrow: 1, display: 'flex', alignItems: 'center', cursor: 'pointer', fontSize: '1.6rem', fontWeight: 'bold' }}>
